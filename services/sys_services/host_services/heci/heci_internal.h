...
#define HECI_MAX_PAYLOAD_SIZE (HECI_IPC_PACKET_SIZE - sizeof(struct heci_hdr_t))
...
struct heci_client_prop_resp_t {
	...
	struct heci_guid_t protocol_id __packed;
...
struct heci_device_t {
	...
	bool dma_req;
	atomic_val_t notify_new_clients;
	atomic_val_t registered_clients;
	atomic_val_t num_of_pse_clients;
