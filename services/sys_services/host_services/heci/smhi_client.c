...
LOG_MODULE_REGISTER(smhi_ia, CONFIG_SMHI_IA_LOG_LEVEL);
...
static void smhi_ret_time(smhi_msg_hdr_t *txhdr)
{
	...
	mrd_t m = { 0 };
	smhi_get_time_resp *resp = (smhi_get_time_resp *)(txhdr + 1);

	memset(resp, 0, sizeof(smhi_get_time_resp));
	txhdr->command = SMHI_GET_TIME;
	/* TODO resp->rtc_us = _rtc_get();*/
...
static void smhi_handle_reset_resp(smhi_msg_hdr_t *txhdr)
{
	...
	smhi_reset_resp *resp = (smhi_reset_resp *)(txhdr + 1);
...
{
	if ((smhi_flag & REBOOT_FLAG) && ((smhi_flag & SMHI_CONN_FLAG) == 0)) {
		smhi_flag &= (~REBOOT_FLAG);
...
static int smhi_init(const struct device *arg)
{
	...
	k_thread_create(&smhi_thread, smhi_stack, SMHI_STACK_SIZE, smhi_task,
