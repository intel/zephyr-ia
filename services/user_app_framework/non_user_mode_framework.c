...
#define DEFINE_INIT_USER_MODE_APP(app_id, na)					  \
	...
	{									  \
		...
		if ((app##app_id##_info.mode & USER_MODE_APP) &&		  \
			...
			LOG_ERR("Invalid App priority given for app_id:%d\n",	  \
				app_id);					  \
			return;							  \
		}								  \
		app##app_id##_tid = k_thread_create(				  \
			app##app_id##_info.app_handle,				  \
			app##app_id##_info.app_stack,				  \
			app##app_id##_info.stack_size,				  \
			app##app_id##_info.user_app_main, NULL, NULL, NULL,	  \
			global_res->priority, 0, K_FOREVER);			  \
...
{
	printk("%s\n", __func__);
	INIT_ALL_USER_MODE_APP(0, 0);
	INVOKE_APP_CONFIG_CALLBACK(0, 0);
	START_USER_MODE_APP(0, 0);
